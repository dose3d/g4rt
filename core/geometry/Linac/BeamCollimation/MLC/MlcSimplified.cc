...
void MlcSimplified::SetRunConfiguration(const ControlPoint* control_point){
    ...
    if(m_fieldShape == "Rectangular" || m_fieldShape == "Elipsoidal"){
        ...
        double inner_leaf_width = 2.5; // mm
        double outer_leaf_width = 5.0; // mm
        int total_leaves = 60;
        int inner_leaf_count = 32;
        int outer_each_side = (total_leaves - inner_leaf_count) / 2;
        double total_width = 220.0;
        double current_x = - total_width / 2.0;
        ...
        for(int leaf_idx = 0; leaf_idx < mlc_a_positioning.size(); leaf_idx++) {
            ...
            m_mlc_a_corners.emplace_back(leaf_a_pos_y, leaf_center_x - x_half_width);
            m_mlc_a_corners.emplace_back(leaf_a_pos_y, leaf_center_x + x_half_width);
