...
void D3DTray::ParseTomlConfig(){
    ...
    if (!svc::checkIfFileExist(configFile)) {
        LOGSVC_FATAL("Geometry","D3DTray::TConfigurarable::ParseTomlConfig::File {} not fount.", configFile);
        exit(1);
    }
    auto config = toml::parse_file(configFile);
    auto configPrefix = GetTomlConfigPrefix();
    LOGSVC_INFO("Geometry","D3DTray::Importing configuration from: {}:{}",configFile,configPrefix);
